...
contains
  ...
  subroutine advance(this,dt,U,V,W,rho,visc,diff,stress_x,stress_y,stress_z,divDiff,T,srcU,srcV,srcW,srcSC,fCp)
    ...
    real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: divDiff   !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
    ...
    do i=1,this%np_
       ...
       do while (dt_done.lt.dt)
          ...
          call this%get_rhs(U=U,V=V,W=W,rho=rho,visc=visc,diff=diff,stress_x=stress_x,stress_y=stress_y,stress_z=stress_z,divDiff=divDiff,T=T,&
  ...
  subroutine get_rhs(this,U,V,W,rho,visc,diff,stress_x,stress_y,stress_z,divDiff,T,p,acc,opt_dt,dTdt,fCp)
    use mathtools, only: Pi
    implicit none
    class(lpt), intent(inout) :: this
    real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: U         !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
    ...
    real(WP) :: fvisc,fdiff,frho,pVF,fVF,fT,tau,Re,divDiff_
    ...
    interpolate: block
      ...
      ! Interpolate the divergence of thermal conduction
      divDiff_=this%cfg%get_scalar(pos=p%pos,i0=p%ind(1),j0=p%ind(2),k0=p%ind(3),S=divDiff,bc='n')
    ...
    compute_heat_transfer: block
      ...
      dTdt=dTdt*3.0_WP*pi/(2.0_WP*theta*6.0_WP)+divDiff_*fCp/(this%rho*this%pCp) ! Correct for EE gas-solid heat transfer coefficient used in Sun/Peng papers
