...
contains
  ...
  subroutine advance(this,dt,U,V,W,rho,visc,diff,stress_x,stress_y,stress_z,T,YCO2,srcU,srcV,srcW,srcSC)
    ...
    integer :: i,j,k,ierr
    real(WP) :: mydt,dt_done,Ip,dmdt,dm,dTdt,dTdt_r,dTemp,mass,fT,fCp
    ...
    do i=1,this%np_
       ...
       do while (dt_done.lt.dt)
          ...
          call this%get_rhs(U=U,V=V,W=W,rho=rho,visc=visc,diff=diff,stress_x=stress_x,stress_y=stress_y,stress_z=stress_z,T=T,YCO2=YCO2,&
               p=myp,acc=acc,opt_dt=myp%dt,dmdt=dmdt,dTdt=dTdt,dTdt_r=dTdt_r,fCp=fCp)
  ...
  subroutine get_rhs(this,U,V,W,rho,visc,diff,stress_x,stress_y,stress_z,T,YCO2,p,acc,opt_dt,dmdt,dTdt,dTdt_r,fCp)
    use mathtools, only: Pi
    implicit none
    class(lpt), intent(inout) :: this
    real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: U         !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
    ...
    real(WP), intent(out) :: opt_dt,dmdt,dTdt,fCp,dTdt_r
    ...
    mass_transfer: block
      ...
      real(WP) :: T_ref,D_ref,D_m,D_k,D_e,d_p,eps_p,tau_p,k_ldf,PCO2_clip,a1,a2,b01,b02,b1,b2,E1R,E2R
      ...
      case('Z13X','z13x')
         ...
         !Estimate k_ldf
         k_ldf=15.0_WP*eps_p*D_e/(tau_p*(p%d/2)**2.0_WP)
