...
module lpt_class
   ...
   integer           , dimension(part_nblock) :: part_lblock=[1,13,4]
   ...
   type :: lpt
      ...
      character(len=str_medium) :: drag_model             !< Drag model
      character(len=str_medium) :: ads_model              !< Adsorption model
      ...
      ! Scalar source
      real(WP), dimension(:,:,:,:), allocatable :: srcSC  !< Scalar source on mesh
      ...
      real(WP), dimension(:,:,:), allocatable :: Tf,Yf !< Filtered temperature and mass fraction
...
contains
   ...
   function constructor(cfg,name) result(self)
      ...
      ! Filtered fields
      allocate(self%Tf(self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_))
      allocate(self%Yf(self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_))
   ...
   subroutine advance(this,dt,U,V,W,rho,visc,T,YCO2)
   !subroutine advance(this,dt,U,V,W,rho,visc)
      ...
      ! Filter fields
      this%Tf=T; call this%filter(this%Tf)
      this%Yf=YCO2; call this%filter(this%Yf)
      ...
      do i=1,this%np_
         ...
         do while (dt_done.lt.dt)
            ...
            call this%get_rhs(U=U,V=V,W=W,rho=rho,visc=visc,T=this%Tf,YCO2=this%Yf,p=this%p(i),acc=acc,opt_dt=this%p(i)%dt,dmdt=dmdt)
            !this%p(i)%pos=pold%pos+0.5_WP*mydt*this%p(i)%vel
            !this%p(i)%vel=pold%vel+0.5_WP*mydt*(acc+this%gravity+this%p(i)%col)
            this%p(i)%Mc =pold%Mc +0.5_WP*mydt*dmdt
            ! Correct with midpoint rule
            call this%get_rhs(U=U,V=V,W=W,rho=rho,visc=visc,T=T,YCO2=YCO2,p=this%p(i),acc=acc,opt_dt=this%p(i)%dt,dmdt=dmdt)
            !this%p(i)%pos=pold%pos+mydt*this%p(i)%vel
            !this%p(i)%vel=pold%vel+mydt*(acc+this%gravity+this%p(i)%col)
            this%p(i)%Mc =pold%Mc +mydt*dmdt
            ! Relocalize
            this%p(i)%ind=this%cfg%get_ijk_global(this%p(i)%pos,this%p(i)%ind)
            ! Send source term back to the mesh
            !dmom=mydt*acc*(this%rho*Pi/6.0_WP*this%p(i)%d**3+this%p(i)%Mc)
            dm=mydt*dmdt
            ...
            call this%cfg%set_scalar(Sp=-dm,pos=this%p(i)%pos,i0=this%p(i)%ind(1),j0=this%p(i)%ind(2),k0=this%p(i)%ind(3),S=this%srcSC(:,:,:,this%ind_CO2),bc='n')
            ! Increment
      ...
      do i=1,this%nscalar
         this%srcSC(:,:,:,i)=this%srcSC(:,:,:,i)/this%cfg%vol; call this%cfg%syncsum(this%srcSC(:,:,:,i)); call this%filter(this%srcSC(:,:,:,i))
      end do
   ...
   subroutine get_rhs(this,U,V,W,rho,visc,T,YCO2,p,acc,opt_dt,dmdt)
      implicit none
      class(lpt), intent(inout) :: this
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: U     !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: V     !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      ...
      mass_transfer: block
        ...
        real(WP) :: Cs,A_d,E_d,dS_d,dH_d,PCO2,Pg,k_d,Keq_d,Cc,dcdt
        ...
        case('LEE','Lee')
           ...
           k_d=A_d*p%T/Cs*exp(-E_d/(Rcst*p%T))*1 ! Multiplied by 10000 for testing
    ...
    subroutine scalar_init(this,sc)
      ...
      allocate(this%srcSC(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_,this%nscalar)); this%srcSC=0.0_WP
