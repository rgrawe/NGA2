...
module simulation
  ...
  real(WP) :: visc,diffusivity,inlet_velocity
...
contains
  ...
  subroutine simulation_run
    ...
    do while (.not.time%done())
       ...
       rhof=sc(1)%rho/(1.0_WP-lp%VF)
       call lp%advance(dt=time%dtmid,U=fs%U,V=fs%V,W=fs%W,rho=rhof,visc=fs%visc,diff=sc(ind_T)%diff,stress_x=resU,stress_y=resV,stress_z=resW,&
            T=sc(ind_T)%SC,YCO2=sc(ind_CO2)%SC,srcU=tmp1,srcV=tmp2,srcW=tmp3,srcSC=srcSClp,fCp=fCp)
       srcUlp=srcUlp+tmp1
       srcVlp=srcVlp+tmp2
       srcWlp=srcWlp+tmp3
       
       ! Compute PTKE and store source terms
       call lp%get_ptke(dt=time%dtmid,Ui=Ui,Vi=Vi,Wi=Wi,visc=fs%visc,rho=rhof,T=SC(ind_T)%sc,fCp=fCp,diff=sc(ind_T)%diff,Y=SC(ind_CO2)%sc,srcU=resU,srcV=resV,srcW=resW,srcT=tmp1,srcY=tmp2)
       srcUlp=srcUlp+resU
       srcVlp=srcVlp+resV
       srcWlp=srcWlp+resW
